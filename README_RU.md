...
| 3 | хук    | `_clean_pull_after_`    | запускается после очистки всех изменений в наблюдаемых репозиториях |
...
| 6 | хук    | `_vendor_folder_after_` | запускается после запуска вложенного компонента make-vendor-folder  |
| 7 | хук    | `_push_result_before`   | запускается до отправки готового результата                         |
| 8 | логика |                         | отправки готового результата                                        |
| 9 | хук    | `_push_result_after_`   | запускается после отправки готового результата                      |
...
К примеру у синхронизации есть хук `_push_result_before` соответственно в папке /scripts/hooks/sync.sh/ при наступлении
вызове события **_push_result_before** будут искаться все файлы начинающиеся с _push_result_before

Пример хука /scripts/hooks/sync.sh/_push_result_before_hello
